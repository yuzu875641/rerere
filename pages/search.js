...
export default function Search() {
  ...
  return (
    <div style={{ backgroundColor: '#181818', color: '#FFFFFF', minHeight: '100vh', padding: '20px' }}>
      <h1 style={{ textAlign: 'center', color: '#FFFFFF' }}>"{q}" の検索結果</h1>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        {results.length === 0 ? (
          <p style={{ textAlign: 'center', color: '#AAAAAA' }}>検索結果が見つかりませんでした。</p>
        ) : (
          results.map(item => (
            <div 
              key={item.videoId || item.authorId || item.playlistId} 
              style={{ 
                display: 'flex', 
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <a 
                  href={
                    item.type === 'video' ? `/watch?v=${item.videoId}` :
                    item.type === 'channel' ? `/channel/${item.authorId}` :
                    `/playlist?list=${item.playlistId}`
                  } 
